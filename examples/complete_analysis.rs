...
async fn main() -> Result<()> {
    ...
    println!("🏢 Found {} large-cap stocks (>$10B)", large_cap_stocks.len());
    
    // Display findings
    StockAnalyzer::print_tickers(&top_performers[..10.min(top_performers.len())], 
    ...
    analysis_candidates.extend(
        top_performers.iter().take(2).map(|t| t.symbol.clone())
    );
    
    // Add top 2 large-cap performers (stable growth)
    analysis_candidates.extend(
        top_large_cap.iter().take(2).map(|t| t.symbol.clone())
    );
    ...
    println!("📊 Selected {} stocks for analysis: {:?}", 
    ...
    for (i, symbol) in analysis_candidates.iter().enumerate() {
        println!("\n{} Analyzing {} ({}/{}) {}", 
                 "=".repeat(20), symbol, i + 1, analysis_candidates.len(), "=".repeat(20));            match analyzer.fetch_stock_data(symbol, start, end).await {
            Ok(stock_data) => {
    ...
    if !analysis_results.is_empty() {
        ...
        println!("{:<8} {:<12} {:<8} {:<8} {:<12}", "Symbol", "Price", "RSI", "Signal", "Recommendation");
        println!("{}", "-".repeat(60));
        ...
        for (symbol, data, indicators) in &analysis_results {
            ...
            println!("{:<8} ${:<11.2} {:<8.1} {:<8} {:<12}", 
                     symbol, price, rsi, signal, recommendation);
        }
        
        println!("\n💡 Investment Insights:");
        let oversold_count = analysis_results.iter()
            .filter(|(_, _, indicators)| indicators.rsi.unwrap_or(50.0) < 30.0)
            .count();
        let overbought_count = analysis_results.iter()
            .filter(|(_, _, indicators)| indicators.rsi.unwrap_or(50.0) > 70.0)
            .count();
            
        println!("   • {} stocks appear oversold (potential buying opportunities)", oversold_count);
        println!("   • {} stocks appear overbought (consider taking profits)", overbought_count);
        println!("   • {} stocks analyzed in total", analysis_results.len());
        
        // Find best opportunities
        let best_opportunities: Vec<_> = analysis_results.iter()
        ...
        if !best_opportunities.is_empty() {
            ...
            for (symbol, data, _) in best_opportunities.iter().take(3) {
                println!("   • {} at ${:.2} - Undervalued with good fundamentals", symbol, data.close);
            }
        }
    } else {
