...
async fn main() -> Result<()> {
    ...
    match StockAnalyzer::fetch_all_tickers().await {
        Ok(all_tickers) => {
            ...
            println!("\n🏢 Found {} large-cap stocks (>$1B market cap)", large_cap_stocks.len());
            ...
            println!("\n🏥 Found {} large-cap Healthcare stocks", healthcare_large_cap.len());
            
            if !healthcare_large_cap.is_empty() {
                let top_healthcare = StockAnalyzer::get_top_performers(&healthcare_large_cap, 5);
                StockAnalyzer::print_tickers(&top_healthcare, "🏥 Top Healthcare Large-Cap Performers");
            }
            ...
            let mut sector_counts: std::collections::HashMap<String, usize> = std::collections::HashMap::new();
            ...
            if !sample_tickers.is_empty() {
                StockAnalyzer::print_tickers(&sample_tickers, "🎯 Sample High-Value Stocks for Analysis");
